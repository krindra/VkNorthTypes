...
class Secure(private val method: suspend (String, Map<Any, Any?>) -> String, private val json: Json) {
    ...
    class SetcounterResponse(private val rawResponse: String, json: Json):
        BaseMultivariateResponse(json) {
        private val response: Any = response()
        private val responsesSerializers = listOf(SecureSetCounterArrayResponse.serializer(), BaseBoolResponse.serializer())
        ...
        fun SecureSetCounterArrayResponse(): SecureSetCounterArrayResponse?{
            return if (response is SecureSetCounterArrayResponse) response else null
        }
